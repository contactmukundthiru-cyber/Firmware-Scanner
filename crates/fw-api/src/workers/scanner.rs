    ...
    match state.scanner.scan_bytes(&data, Some(file_path.clone())) {
        Ok(result) => {
            ...
            for finding in &result.analysis.all_findings {
                ...
                let _ = sqlx::query!(
                    r#"
                    INSERT INTO findings (id, scan_id, capability_type, severity, is_dormant, evidence_json, created_at)
                    VALUES ($1, $2, $3, $4, $5, $6, NOW())
                    "#,
                    Uuid::new_v4(),
                    scan_id,
                    finding.capability_type.to_string(),
            ...
            for verdict in &result.claim_verdicts {
                ...
                let _ = sqlx::query!(
                    r#"
                    INSERT INTO claim_verdicts (id, scan_id, claim_type, compatible, failing_conditions, evidence_ids)
                    VALUES ($1, $2, $3, $4, $5, $6)
                    "#,
                    Uuid::new_v4(),
                    scan_id,
                    verdict.claim.name(),
                    match verdict.compatible {
            ...
            for evidence in &result.evidence {
                let _ = sqlx::query!(
                    r#"
                    INSERT INTO evidence_artifacts (id, scan_id, file_path, byte_offset, byte_length, content_hash, context_data, reproduction_script)
                    VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
                    "#,
                    evidence.id,
                    scan_id,
                    evidence.file_path,
                    evidence.byte_offset as i64,
                    evidence.byte_length as i64,
                    evidence.content_hash,
                    Some(&evidence.matched_data[..]),
                    Some(&evidence.reproduction_notes)
                )
                .execute(&state.db)
                .await;
            }
